...
def parse_args():
    parser = argparse.ArgumentParser(description="Train ResNet101 U-Net on BUSI split 3")
    ...
    parser.add_argument("--output_dir", default="./runs/resnet101_unet_split3")
...
def main():
    ...
    fields = ["epoch", "train_loss", "train_iou", "train_dice", "val_loss",
              "val_iou", "val_dice", "encoder_lr", "decoder_lr"]
    best_iou = -1.0
    best_epoch = 0
    started = time.time()
    with open(os.path.join(args.output_dir, "history.csv"), "w", newline="", encoding="utf-8") as file:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            row = {"epoch": epoch, "encoder_lr": optimizer.param_groups[0]["lr"],
            ...
            scheduler.step()
            print("epoch [{}/{}] val_IoU={:.4f} val_Dice={:.4f}".format(
                epoch, args.epochs, val_metrics["iou"], val_metrics["dice"]), flush=True)
    summary = {"best_epoch": best_epoch, "best_val_iou": best_iou,
        "parameters": sum(p.numel() for p in model.parameters()),
        "loaded_encoder_tensors": model.loaded_encoder_tensors,
        "elapsed_seconds": time.time() - started, "args": vars(args)}
