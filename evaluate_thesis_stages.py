...
def main():
    parser = argparse.ArgumentParser(description="Evaluate a progressive thesis model")
    ...
    model = ThesisFourStageUNet(args.variant).to(device)
    loaded, missing, unexpected = model.load_progressive_checkpoint(args.model_path)
    model.eval()
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            metrics = confusion_metrics(outputs["segmentation"], labels, 0.5)
            samples += images.size(0)
            for name in METRIC_NAMES:
                totals[name] += metrics[name].sum().item()
            for item, index in enumerate(batch["idx"].tolist()):
                rows.append({
                    "case": dataset.sample_list[index],
                    "iou": metrics["iou"][item].item(),
                    "dice": metrics["dice"][item].item(),
                })
    results = {
        ...
        "iou": totals["iou"] / samples,
        "dice": totals["dice"] / samples,
    }
    with open(os.path.join(args.output_dir, "per_case_metrics.csv"), "w", newline="", encoding="utf-8") as file:
        writer = csv.DictWriter(file, fieldnames=("case", "iou", "dice"))
        writer.writeheader()
